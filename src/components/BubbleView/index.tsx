...
void main() {
    ...
    if (gl_VertexID > 0) {  vec3 rotationAxis = uRotationAxisVelocity.xyz;  float rotationVelocity = min(.15, uRotationAxisVelocity.w * 15.);  vec3 stretchDir = normalize(cross(centerPos, rotationAxis));  vec3 relativeVertexPos = normalize(worldPosition.xyz - centerPos);  float strength = dot(stretchDir, relativeVertexPos);  float invAbsStrength = min(0., abs(strength) - 1.);  strength = rotationVelocity * sign(strength) * abs(invAbsStrength * invAbsStrength * invAbsStrength + 1.);  worldPosition.xyz += stretchDir * strength;
    }
...
class Geometry {
    ...
    subdivide(divisions = 1) {
        ...
        for (let div = 0; div < divisions; ++div) {
            ...
            f.forEach((face, ndx) => {
                ...
                const i = ndx * 4; newFaces[i + 0] = new Face(face.a, mAB, mCA); newFaces[i + 1] = new Face(face.b, mBC, mAB); newFaces[i + 2] = new Face(face.c, mCA, mBC); newFaces[i + 3] = new Face(mAB, mBC, mCA);
            });
            f = newFaces;
        }
    ...
    spherize(radius = 1) {
        this.vertices.forEach(vertex => {
            vec3.normalize(vertex.normal, vertex.position); vec3.scale(vertex.position, vertex.normal, radius);
        });
        return this;
    }
    ...
    get data() {
        return {
            vertices: this.vertexData, indices: this.indexData, normals: this.normalData, uvs: this.uvData
        };
    }
...
class DiscGeometry extends Geometry {
    constructor(steps = 4, radius = 1) {
        ...
        for (let i = 0; i < steps; ++i) {
            const x = Math.cos(alpha * i); const y = Math.sin(alpha * i); this.addVertex(radius * x, radius * y, 0); this.lastVertex.uv[0] = x * 0.5 + 0.5; this.lastVertex.uv[1] = y * 0.5 + 0.5;
...
class ArcballControl {
    ...
    constructor(canvas, updateCallback) {
        ...
        canvas.addEventListener('pointerdown', e => {
            vec2.set(this.pointerPos, e.clientX, e.clientY); vec2.copy(this.previousPointerPos, this.pointerPos); this.isPointerDown = true;
        });
    ...
    update(deltaTime, targetFrameDuration = 16) {
        ...
        if (this.isPointerDown) {
            ...
            const midPointerPos = vec2.sub(vec2.create(), this.pointerPos, this.previousPointerPos); vec2.scale(midPointerPos, midPointerPos, INTENSITY);
        ...
        let rv = 0;
        if (s > 0.000001) {
            rv = rad / (2 * Math.PI); this.rotationAxis[0] = this._combinedQuat[0] / s; this.rotationAxis[1] = this._combinedQuat[1] / s; this.rotationAxis[2] = this._combinedQuat[2] / s;
        }
...
class InfiniteGridMenu {
    ...
    camera = {
        ...
        matrices: {
            view: mat4.create(), projection: mat4.create(), inversProjection: mat4.create()
        }
    };
    ...
    #initDiscInstances(count) {
        const gl = this.gl;
        this.discInstances = {
            matricesArray: new Float32Array(count * 16), matrices: [], buffer: gl.createBuffer()
        };
        for (let i = 0; i < count; ++i) {
            const instanceMatrixArray = new Float32Array(this.discInstances.matricesArray.buffer, i * 16 * 4, 16); instanceMatrixArray.set(mat4.create()); this.discInstances.matrices.push(instanceMatrixArray);
        }
    ...
    #render() {
        ...
        gl.clearColor(0, 0, 0, 0); // Tam siyah ve opak arka plan
    ...
    #onControlUpdate(deltaTime) {
        ...
        if (!this.control.isPointerDown) {
            const nearestVertexIndex = this.#findNearestVertexIndex(); const itemIndex = nearestVertexIndex % Math.max(1, this.items.length); this.onActiveItemChange(itemIndex); const snapDirection = vec3.normalize(vec3.create(), this.#getVertexWorldPosition(nearestVertexIndex)); this.control.snapTargetDirection = snapDirection;
        } else {
            cameraTargetZ += this.control.rotationVelocity * 80 + 2.5; damping = 7 / timeScale;
        }
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%' }}><canvas id="infinite-grid-menu-canvas" ref={canvasRef} />
            {activeItem && (<>
                <div className={(isMoving ? 'hidden' : '') + ` absolute flex flex-col gap-2 items-center justify-center text-center bottom-0  w-full  h-1/4 md:h-1/7  z-5 rounded-lg backdrop-blur-sm py-5 bg-red-500"`}>
                    <h2 className="text-3xl font-bold">{activeItem?.username} <span className="font-light">{calculateAge(activeItem.date_of_birth)}</span></h2>
                    <ActionBar
                        viewMode='bubble'
                        liked={liked}
                        disliked={disliked}
                        ...
                        onBlockToggle={() => {
                            ...
                            handleBlock(activeItem)
                        }}
                        onLikeToggle={() => {
                            setLiked((prev) => !prev)
                            handleSendLike(activeItem)
                        }}
                        onDislikeToggle={() => {
                            setDisliked((prev) => !prev)
                            handleSendDislike(activeItem)
                        }}
                        onOpenGiftSelector={() => setIsGiftSelectorOpen(true)}
                        onOpenQuickMessageSelector={() => {
                            handleSendMessage(activeItem)
                        }}
